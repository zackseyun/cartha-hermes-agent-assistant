...
## Decision-first report rendering

`desktop/report/` renders an explicitly requested Graphic Bible report from its recorded `get_growth_metrics` tool result, not by extracting numbers from the assistant's prose. It works on existing conversations. One short takeaway, three metrics, cohort explanation and two next actions replace the dense primary transcript. Activity counts, timestamps and limitations are expandable; the original answer and tool activity remain available in a separate disclosure. Nothing is deleted or rewritten in the session database.

The formatter stays inside the founder profile and the current user turn. General founder/reliability reports, other profiles, running messages, missing data and malformed tool envelopes retain their normal transcript presentation. Null mature conversion remains an em dash, not zero. Mature rates are derived from mature numerator/denominator counts. Old snapshots carry a freshness warning. Rendering cannot issue network requests, execute HTML or access secrets.
