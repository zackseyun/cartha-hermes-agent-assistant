...
export function composeAgentPrompt({ registry, agentId, projectIds, task, worktree = true }) {
  ...
  const projectSections = selected.map((project, index) => [
    ...
    `Access in this run: ${index === 0 ? (worktree ? "PRIMARY — edits allowed in the isolated worktree" : "PRIMARY — current checkout; edits allowed only after confirming it is clean") : "SECONDARY — read-only dependency context; use a separate agent run before editing"}`,
  ...
  return [
    ...
    "- Only the first listed project is writable in this run. Treat every secondary project as read-only dependency context and open a separate scoped run before editing it.",
